...
import { findNearbyVeterinaryClinics } from '../services/vetLocatorService';
...
const fallbackClinics: VeterinaryClinic[] = vetClinics.map(clinic => ({
  ...clinic,
  source: 'static',
}));
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center gap-2 mt-1">
              <span className="text-xs font-bold uppercase tracking-widest text-brand-brown-400">
                {clinic.source === 'osm' ? 'Live OSM result' : 'Fallback list'}
              </span>
              {clinic.distanceKm !== undefined && (
                <span className="text-xs font-bold text-brand-green-700 dark:text-brand-green-300">
        ...
        {clinic.phone ? (
          <div className="flex items-center gap-2 pl-10">
            ...
            <a href={`tel:${clinic.phone}`} className="text-md font-bold text-brand-green-700 dark:text-brand-green-300 hover:underline">
              {clinic.phone}
            </a>
          </div>
        ) : (
          <p className="text-sm text-brand-brown-500 pl-10">Phone number not listed in source data.</p>
        )}

        {clinic.address && (
          <p className="text-sm text-brand-brown-600 dark:text-brand-brown-300 pl-10">{clinic.address}</p>
        )}

        <p className="text-xs font-medium text-brand-brown-400 pl-10 tracking-widest uppercase">
          Coordinates: {clinic.lat.toFixed(4)}, {clinic.lon.toFixed(4)}
        </p>
      </div>
...
export const VetsNearby: React.FC<VetsNearbyProps> = ({ onBack, language }) => {
  ...
  useEffect(() => {
    ...
    const loadNearbyClinics = async (latitude: number, longitude: number) => {
      try {
        const liveClinics = await findNearbyVeterinaryClinics(latitude, longitude);
        ...
        if (liveClinics.length === 0) {
          useFallback('No live veterinary listings were returned nearby, so the app is showing its offline fallback list.');
          return;
        }

        setClinics(liveClinics);
        setIsSearching(false);
      } catch (error) {
        useFallback(
          `Live veterinary lookup failed${error instanceof Error ? `: ${error.message}` : '.'} Showing offline fallback results instead.`
        );
      }
    };
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        position => {
          ...
          void loadNearbyClinics(position.coords.latitude, position.coords.longitude);
        },
        error => {
    ...
    <div className="w-full max-w-4xl text-left animate-fade-in mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6">
        <div className="flex items-center">
          ...
          <div>
            <span className="text-brand-green-600 dark:text-brand-green-400 font-black uppercase tracking-widest text-xs mb-1 block">
              Localized Support
            </span>
      ...
      {isSearching ? (
        <div className="text-center p-12 flex flex-col items-center justify-center min-h-[40vh] bg-white dark:bg-brand-brown-900 rounded-[40px] shadow-sm border border-gray-100 dark:border-brand-brown-800">
          ...
          <p className="text-brand-brown-500 mt-2 font-medium">Fetching nearby veterinary listings from OpenStreetMap...</p>
        </div>
      ) : (
        <div className="space-y-6">
          {locationError && (
            <div className="bg-amber-50 border-l-4 border-amber-500 text-amber-800 p-6 rounded-2xl flex items-center gap-4" role="alert">
          ...
          {apiError && (
            <div className="bg-blue-50 border-l-4 border-blue-500 text-blue-900 p-6 rounded-2xl flex items-center gap-4" role="status">
              <Icon name="info" className="w-6 h-6 flex-shrink-0" />
              <div>
                <p className="font-black text-lg">Live Lookup Status</p>
                <p className="font-medium opacity-80">{apiError}</p>
              </div>
            </div>
          )}
          ...
          {filteredVets.length > 0 ? (
            <div className="grid grid-cols-1 gap-6 animate-fade-in">
              {filteredVets.map(clinic => (
                <VetCard key={`${clinic.name}-${clinic.lat}-${clinic.lon}`} clinic={clinic} language={language} />
              ))}
            </div>
          ) : (
            <div className="text-center py-24 bg-white dark:bg-brand-brown-900 rounded-[40px] border border-dashed border-brand-brown-200 dark:border-brand-brown-800">
              ...
              <button onClick={() => setSearchTerm('')} className="mt-6 text-brand-green-600 font-bold hover:underline">
                Clear search
              </button>
            </div>
          )}
