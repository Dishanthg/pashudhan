...
const App: React.FC = () => {
  ...
  const handleLogin = async (username: string, password: string) => {
    const user = password.split('.').length === 3
      ? await authService.loginWithGoogle(password)
  ...
  const renderView = () => {
    switch (mainView) {
      ...
      case 'semen': return <HerdManagement herd={herd} onBack={() => navigateTo('dashboard')} language={language} />;
  ...
  if (!currentUser) {
      return (
        <div className="app-shell flex min-h-screen flex-col text-brand-brown-900 dark:text-brand-brown-200">
            ...
            <main className="relative z-10 flex min-h-screen flex-grow items-center justify-center">
  ...
  return (
    <div className="app-shell flex min-h-screen flex-col text-brand-brown-900 dark:text-brand-brown-200">
        ...
        />
        <div className="relative z-10 w-full flex-grow pt-[76px]">
